...
def trigger_job_execution(
    ...
    *,
    ...
    - `force_full` → `OBSERVABILITY_HUB_CACHE_FORCE_FULL=1`: full scan da
      janela inteira em vez do delta incremental.
    - `only_projects` → `OBSERVABILITY_HUB_CACHE_ONLY_PROJECTS=a,b`: roda
    ...
    if force_full:
        env.append(run_v2.EnvVar(name="OBSERVABILITY_HUB_CACHE_FORCE_FULL", value="1"))
    if only_projects:
        env.append(
            run_v2.EnvVar(
                name="OBSERVABILITY_HUB_CACHE_ONLY_PROJECTS", value=",".join(only_projects)
            )
        )
    if not env:
