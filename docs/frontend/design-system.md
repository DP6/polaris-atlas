...
| `Funnel` | Funil de retenção: barras horizontais **centradas** afunilando de cima pra baixo (largura ∝ contagem), rótulo+valor+% acima de cada barra. Sem SVG esticado (rodada 3 — a versão `<polygon>` distorcia dentro de container de altura fixa). `role="img"` + `aria-label` + `<table>` sr-only. | Comparação de categorias não afuniladas (use `BarChart`) |
