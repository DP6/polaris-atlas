...
| 2026-08-21 | `dp6-ci-polaris` | `backend-dev-run@...` e `backend-prod-run@...` (self, nível de recurso da própria SA — não nível de projeto) | `roles/iam.serviceAccountTokenCreator` concedida por cada SA a si mesma — permite assinar o JWT de domain-wide delegation (`google.auth.iam.Signer`/`signBlob`) sem precisar de chave baixada. **Pendente de confirmação**: a propagação passou de 7 minutos sem efeito nas checagens (`iam.serviceAccounts.getIamPolicy` seguia negada) — retry em background numa sessão desta data; confirmar com `gcloud iam service-accounts get-iam-policy <SA>` antes de considerar concluído | Ver ressalva acima — não confirmado nesta sessão |
