...
| 2026-08-21 | `dp6-ci-polaris` (repo real `polaris-hub-gcp`, projeto single dev+prod) | `gh-deploy-dev@...` e `gh-deploy-prod@...` (a confirmar quais das duas — TI relatou "IAP ativado" sem especificar) | `roles/iap.admin` — reportada como concedida pela TI, substituindo o pedido anterior de `roles/run.invoker` a `allUsers` (bloqueado pela Org Policy `iam.allowedPolicyMemberDomains`) | **Contradito por evidência real**: `Terraform Plan`/`Apply (dev)` no CI (PR #1, runs 32501467924/32501493798) falharam com 403 "the caller does not have permission" tentando ler/escrever `google_iap_web_cloud_run_service_iam_member` — ou a role não chegou em `gh-deploy-dev@...` especificamente, ou não propagou. Pendente reconfirmação com a TI antes do PR passar. |
