...
| 2026-08-21 | `dp6-ci-polaris` (repo real `polaris-hub-gcp`, projeto single dev+prod) | `gh-deploy-dev@...` e `gh-deploy-prod@...` (a confirmar quais das duas — TI relatou "IAP ativado" sem especificar) | `roles/iap.admin` concedida pela TI, substituindo o pedido anterior de `roles/run.invoker` a `allUsers` (bloqueado pela Org Policy `iam.allowedPolicyMemberDomains`) — habilita IAP nativo no Cloud Run (sem Load Balancer) como mecanismo de acesso | **Não confirmado via `gcloud`** — relatado pelo usuário nesta sessão, sem sessão `gcloud` autenticada disponível para verificar `get-iam-policy`. Validar no próximo `terraform plan`/`apply` real. |
| 2026-08-21 | `dp6-ci-polaris` | — | Google Group `gcp-ci-polaris@dp6.com.br` criado pela TI — recebe `roles/iap.httpsResourceAccessor` nos 4 serviços Cloud Run (backend/frontend × dev/prod) via `google_iap_web_cloud_run_service_iam_member`, substitui o antigo `allUsers` como controle de acesso | Não confirmado via `gcloud` — mesma ressalva da linha acima |
